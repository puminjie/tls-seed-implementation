...
{	
  ...
  gettimeofday( &tv, 0 );
	...
	hs_start = get_current_millis();
	RECORD_LOG(time_log, CLIENT_HANDSHAKE_START);
  ...
  if ( (ret = SSL_connect(ssl)) < 0 )   /* perform the connection */
  {
	...
	{
		RECORD_LOG(time_log, CLIENT_HANDSHAKE_END);
		INTERVAL(time_log, CLIENT_HANDSHAKE_START, CLIENT_HANDSHAKE_END);
		hs_end = get_current_millis();
...
{   
  ...
  //SSL_CTX_set_cipher_list(ctx, "AEAD-AES128-GCM-SHA256");
