...
#ifndef OPENSSL_NO_TLS_DC
# define DELEGATED_CREDENTIAL_LENGTH_MAX 256
#endif /* OPENSSL_NO_TLS_DC */
...
# define SSL_CTRL_CHAIN_ORIG_CERT                134
...
# define SSL_CTX_add0_chain_orig_cert(ctx,x509) \
        SSL_CTX_ctrl(ctx,SSL_CTRL_CHAIN_ORIG_CERT,0,(char *)(x509))
# define SSL_CTX_add1_chain_orig_cert(ctx,x509) \
        SSL_CTX_ctrl(ctx,SSL_CTRL_CHAIN_ORIG_CERT,1,(char *)(x509))
...
# define SSL_add0_chain_orig_cert(s,x509) \
        SSL_ctrl(s,SSL_CTRL_CHAIN_ORIG_CERT,0,(char *)(x509))
# define SSL_add1_chain_orig_cert(s,x509) \
        SSL_ctrl(s,SSL_CTRL_CHAIN_ORIG_CERT,1,(char *)(x509))
...
#ifdef TIME_LOG
void SSL_set_time_logger(SSL *s, logger_t *logger);
logger_t *SSL_get_time_logger(SSL *s);
#endif /* TIME_LOG */

#ifndef OPENSSL_NO_TLS_SEED
int SSL_use_orig_certificate(SSL *ssl, X509 *x);
int SSL_use_orig_certificate_file(SSL *ssl, const char *file, int type);
int SSL_use_orig_certificate_ASN1(SSL *ssl, const unsigned char *d, int len);
int SSL_use_cc_mem(SSL *ssl, const uint8_t *cc, size_t cclen);

void SSL_CTX_set_get_seed_digest(SSL_CTX *ctx, 
    int (*cb)(uint8_t *digest, uint16_t *ht, size_t *len));
uint8_t *SSL_get_seed_digest(SSL *ssl, uint32_t *seed_digest_length);
void SSL_set_get_seed_digest(SSL *ssl, int (*cb)(uint8_t *digest, uint16_t *ht, size_t *len));
void SSL_set_seed_digest(SSL *ssl);
void SSL_set_seed_digest_buf(SSL *ssl, uint8_t *digest, uint16_t ht, uint32_t len);

void SSL_CTX_enable_seed(SSL_CTX *ctx);
void SSL_CTX_disable_seed(SSL_CTX *ctx);
void SSL_enable_seed(SSL *s);
void SSL_disable_seed(SSL *s);
int SSL_check_seed_enabled(SSL *s);

void SSL_CTX_set_fallback(SSL_CTX *ctx, int fallback);
void SSL_set_fallback(SSL *ssl, int fallback);
int SSL_check_fallback(SSL *ssl);
#endif /* OPENSSL_NO_TLS_SEED */

#ifndef OPENSSL_NO_TLS_DC
void SSL_CTX_enable_dc(SSL_CTX *ctx);
void SSL_CTX_disable_dc(SSL_CTX *ctx);
void SSL_enable_dc(SSL *s);
void SSL_disable_dc(SSL *s);
int SSL_check_dc_enabled(SSL *s);

int SSL_use_PrivateKey_dc(SSL *s, EVP_PKEY *pkey);
int SSL_use_PrivateKey_dc_mem(SSL *s, const unsigned char *dc_priv, size_t dc_priv_len, 
    int type);
int SSL_use_PrivateKey_dc_file(SSL *s, const char *file, int type);
int SSL_use_delegated_credential_mem(SSL *s, const unsigned char *dc, size_t dclen);
int SSL_use_delegated_credential_file(SSL *s, const char *file);
#endif /* OPENSSL_NO_TLS_DC */

#ifndef OPENSSL_NO_KEYLESS_SSL
void SSL_CTX_enable_keyless_ssl(SSL_CTX *ctx);
void SSL_CTX_disable_keyless_ssl(SSL_CTX *ctx);
void SSL_CTX_set_keyless_ssl_callback(SSL_CTX *ctx,
    int (*cb)(SSL *ssl, uint8_t *out, uint8_t *in, uint32_t ilen, size_t *slen));
void SSL_enable_keyless_ssl(SSL *s);
void SSL_disable_keyless_ssl(SSL *s);
int SSL_check_keyless_ssl_enabled(SSL *s);
void SSL_set_keyless_ssl_callback(SSL *ssl,
    int (*cb)(SSL *ssl, uint8_t *out, uint8_t *in, uint32_t ilen, size_t *slen));
void SSL_set_keyless_ssl_arg(SSL *ssl, void *arg);
void *SSL_get_keyless_ssl_arg(SSL *ssl);
#endif /* OPENSSL_NO_KEYLESS_SSL */

#ifndef OPENSSL_NO_MBTLS
void SSL_CTX_enable_mbtls(SSL_CTX *ctx);
void SSL_CTX_disable_mbtls(SSL_CTX *ctx);
void SSL_enable_mbtls(SSL *s);
void SSL_disable_mbtls(SSL *s);
int SSL_check_mbtls_enabled(SSL *s);
#endif /* OPENSSL_NO_MBTLS */

#ifndef OPENSSL_NO_SPX
void SSL_CTX_enable_spx(SSL_CTX *ctx);
void SSL_CTX_disable_spx(SSL_CTX *ctx);
void SSL_enable_spx(SSL *s);
void SSL_disable_spx(SSL *s);
int SSL_check_spx_enabled(SSL *s);
#endif /* OPENSSL_NO_SPX */
////////////////////
