...
///// Add for tls-ec /////
int tls13_process_tlsec_certificate(SSL_HANDSHAKE *hs, const SSLMessage &msg,
                              int allow_anonymous) {
  EDGE_LOG("Start: tls13_process_tlsec_certificate");
  SSL *const ssl = hs->ssl;
  CBS body = msg.body;
  bssl::UniquePtr<CRYPTO_BUFFER> decompressed;
  ...
  /* Confirm the EC certificate */
  EDGE_MSG("tls13: before the EC certificate");
  RECORD_LOG(ssl->time_log, CLIENT_TLS13_VERIFY_EC_CERTIFICATE_START);
  CBS context, certificate_list;
  if (!CBS_get_u8_length_prefixed(&body, &context) ||
      CBS_len(&context) != 0 ||
      !CBS_get_u24_length_prefixed(&body, &certificate_list)) {
  ...
  RECORD_LOG(ssl->time_log, CLIENT_TLS13_VERIFY_EC_CERTIFICATE_END);
  INTERVAL(ssl->time_log, CLIENT_TLS13_VERIFY_EC_CERTIFICATE_START, CLIENT_TLS13_VERIFY_EC_CERTIFICATE_END);

  /* Confirm the origin's certificate */
  EDGE_MSG("tls13: before the origin's certificate");
  RECORD_LOG(ssl->time_log, CLIENT_TLS13_VERIFY_ORIG_CERTIFICATE_START);
  CBS orig_context, orig_certificate_list;
  if (!CBS_get_u8_length_prefixed(&body, &orig_context) ||
      CBS_len(&orig_context) != 0 ||
      !CBS_get_u24_length_prefixed(&body, &orig_certificate_list)) {
    ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_DECODE_ERROR);
    OPENSSL_PUT_ERROR(SSL, SSL_R_DECODE_ERROR);
    return 0;
  }

  UniquePtr<STACK_OF(CRYPTO_BUFFER)> orig_certs(sk_CRYPTO_BUFFER_new_null());
  if (!orig_certs) {
    ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_INTERNAL_ERROR);
    OPENSSL_PUT_ERROR(SSL, ERR_R_MALLOC_FAILURE);
    return 0;
  ...
  UniquePtr<EVP_PKEY> orig_pkey;
  while (CBS_len(&orig_certificate_list) > 0) {
    CBS certificate, extensions;
    if (!CBS_get_u24_length_prefixed(&orig_certificate_list, &certificate) ||
        !CBS_get_u16_length_prefixed(&orig_certificate_list, &extensions) ||
        CBS_len(&certificate) == 0) {
      ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_DECODE_ERROR);
      OPENSSL_PUT_ERROR(SSL, SSL_R_CERT_LENGTH_MISMATCH);
      return 0;
    ...
    if (sk_CRYPTO_BUFFER_num(orig_certs.get()) == 0) {
      orig_pkey = ssl_cert_parse_pubkey(&certificate);
      if (!orig_pkey) {
        ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_DECODE_ERROR);
        OPENSSL_PUT_ERROR(SSL, SSL_R_DECODE_ERROR);
        return 0;
    ...
    UniquePtr<CRYPTO_BUFFER> buf(
        CRYPTO_BUFFER_new_from_CBS(&certificate, ssl->ctx->pool));
    if (!buf ||
        !PushToStack(orig_certs.get(), std::move(buf))) {
      ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_INTERNAL_ERROR);
      OPENSSL_PUT_ERROR(SSL, ERR_R_MALLOC_FAILURE);
      return 0;
    }
    ...
    if (have_status_request) {
      ...
      if (sk_CRYPTO_BUFFER_num(orig_certs.get()) == 1) {
        hs->new_session->ocsp_response.reset(
            CRYPTO_BUFFER_new_from_CBS(&ocsp_response, ssl->ctx->pool));
        if (hs->new_session->ocsp_response == nullptr) {
          ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_INTERNAL_ERROR);
  ...
  hs->orig_pubkey = std::move(orig_pkey);
  hs->new_session->orig_certs = std::move(orig_certs);
  ...
  if (sk_CRYPTO_BUFFER_num(hs->new_session->orig_certs.get()) == 0) {
    if (!allow_anonymous) {
      OPENSSL_PUT_ERROR(SSL, SSL_R_PEER_DID_NOT_RETURN_A_CERTIFICATE);
      ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_CERTIFICATE_REQUIRED);
      return 0;
  ...
  RECORD_LOG(ssl->time_log, CLIENT_TLS13_VERIFY_ORIG_CERTIFICATE_END);
  INTERVAL(ssl->time_log, CLIENT_TLS13_VERIFY_ORIG_CERTIFICATE_START, CLIENT_TLS13_VERIFY_ORIG_CERTIFICATE_END);

  /* Confirm the software assertion */
  uint8_t alert;
  EDGE_MSG("tls13: before software assertion");
  RECORD_LOG(ssl->time_log, CLIENT_TLS13_VERIFY_SA_START);
  CBS sa;
  if (!CBS_get_u16_length_prefixed(&body, &sa))
  {
    alert = SSL_AD_SA_DECODE_ERROR;
    ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_SA_DECODE_ERROR);
    return 0;
  }
  ...
  RECORD_LOG(ssl->time_log, CLIENT_TLS13_VERIFY_SA_END);
  INTERVAL(ssl->time_log, CLIENT_TLS13_VERIFY_SA_START, CLIENT_TLS13_VERIFY_SA_END);

  /* Confirm the cross credential */
  EDGE_MSG("tls13: before cross credential");
  RECORD_LOG(ssl->time_log, CLIENT_TLS13_VERIFY_CC_START);
  CBS cc;
  ...
  RECORD_LOG(ssl->time_log, CLIENT_TLS13_VERIFY_CC_END);
  INTERVAL(ssl->time_log, CLIENT_TLS13_VERIFY_CC_START, CLIENT_TLS13_VERIFY_CC_END);
...
int tls13_process_certificate_verify(SSL_HANDSHAKE *hs, const SSLMessage &msg) {
  ...
  EDGE_LOG("Finished: tls13_tlsec_process_certificate");
  return 1;
}
...
int tls13_tlsec_add_certificate(SSL_HANDSHAKE *hs) {
  EDGE_LOG("Start: tls13_tlsec_add_certificate");
  SSL *const ssl = hs->ssl;
  CERT *const cert = hs->config->cert.get();
  CERT *const orig_cert = hs->config->orig_cert.get();

  ScopedCBB cbb;
  CBB *body, body_storage, certificate_list, orig_certificate_list;
  ...
  EDGE_LOG("Add the EC's certificate");
  /* Add the EC's certificate */
  if (// The request context is always empty in the handshake.
      !CBB_add_u8(body, 0) ||
      !CBB_add_u24_length_prefixed(body, &certificate_list)) {
    OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
  ...
  EDGE_LOG("Add the origin's certificate");
  /* Add the origin's certificate */
  if (// The request context is always empty in the handshake.
      !CBB_add_u8(body, 0) ||
      !CBB_add_u24_length_prefixed(body, &orig_certificate_list)) {
    OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
    return 0;
  }
  ...
  CRYPTO_BUFFER *orig_leaf_buf = sk_CRYPTO_BUFFER_value(orig_cert->chain.get(), 0);
  CBB orig_leaf, orig_extensions;
  if (!CBB_add_u24_length_prefixed(&orig_certificate_list, &orig_leaf) ||
      !CBB_add_bytes(&orig_leaf, CRYPTO_BUFFER_data(orig_leaf_buf),
                     CRYPTO_BUFFER_len(orig_leaf_buf)) ||
      !CBB_add_u16_length_prefixed(&orig_certificate_list, &orig_extensions)) {
    OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
    return 0;
  }

  if (hs->scts_requested && orig_cert->signed_cert_timestamp_list != nullptr) {
    CBB contents;
    if (!CBB_add_u16(&orig_extensions, TLSEXT_TYPE_certificate_timestamp) ||
        !CBB_add_u16_length_prefixed(&orig_extensions, &contents) ||
        !CBB_add_bytes(
            &contents,
            CRYPTO_BUFFER_data(orig_cert->signed_cert_timestamp_list.get()),
            CRYPTO_BUFFER_len(orig_cert->signed_cert_timestamp_list.get())) ||
        !CBB_flush(&orig_extensions)) {
      OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
      return 0;
    }
  }

  if (hs->ocsp_stapling_requested && orig_cert->ocsp_response != NULL) {
    CBB contents, ocsp_response;
    if (!CBB_add_u16(&orig_extensions, TLSEXT_TYPE_status_request) ||
        !CBB_add_u16_length_prefixed(&orig_extensions, &contents) ||
        !CBB_add_u8(&contents, TLSEXT_STATUSTYPE_ocsp) ||
        !CBB_add_u24_length_prefixed(&contents, &ocsp_response) ||
        !CBB_add_bytes(&ocsp_response,
                       CRYPTO_BUFFER_data(orig_cert->ocsp_response.get()),
                       CRYPTO_BUFFER_len(orig_cert->ocsp_response.get())) ||
        !CBB_flush(&orig_extensions)) {
      OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
      return 0;
    }
  ...
  for (size_t i = 1; i < sk_CRYPTO_BUFFER_num(orig_cert->chain.get()); i++) {
    CRYPTO_BUFFER *cert_buf = sk_CRYPTO_BUFFER_value(orig_cert->chain.get(), i);
    CBB child;
    if (!CBB_add_u24_length_prefixed(&orig_certificate_list, &child) ||
        !CBB_add_bytes(&child, CRYPTO_BUFFER_data(cert_buf),
                       CRYPTO_BUFFER_len(cert_buf)) ||
        !CBB_add_u16(&orig_certificate_list, 0 /* no extensions */)) {
      OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
      return 0;
    }
  }

  EDGE_LOG("Add the software assertion");
  /* Add software assertion */
  ...
  if (!CBB_add_u16_length_prefixed(body, &sa))
  {
    OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
    return 0;
  }
  ...
  EDGE_LOG("Add the cross credential");
  /* Add cross credential */
  ...
  CBB cc;
  if (!CBB_add_u16_length_prefixed(body, &cc))
  {
    OPENSSL_PUT_ERROR(SSL, ERR_R_INTERNAL_ERROR);
    return 0;
  }
  ...
  EDGE_LOG("Finished: tls13_tlsec_add_certificate");
  return 1;
}
...
enum ssl_private_key_result_t tls13_add_certificate_verify(SSL_HANDSHAKE *hs) {
  ...
  if (ssl->keyless_ssl)
  {
    ...
    {
      ssl->keyless_ssl_callback(ssl, sig, msg.data(), msg.size(), &sig_len);
    }
  }
  else
  {
