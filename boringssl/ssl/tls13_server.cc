...
static enum ssl_hs_wait_t do_send_server_hello(SSL_HANDSHAKE *hs) {
  ...
  if (!ssl->s3->session_reused) {
    ...
    if (ssl->ec_enabled)
    {
      if (!tls13_tlsec_add_certificate(hs))
      {
        return ssl_hs_error;
      }
    }
  ...
  while (hs->tls13_state != state_done) {
    ...
    switch (state) {
      case state_select_parameters:
        EDGE_MSG("tls13: before do_select_parameters");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_SELECT_PARAMETERS_START);
        ret = do_select_parameters(hs);
        EDGE_MSG("tls13: after do_select_parameters");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_SELECT_PARAMETERS_END);
        INTERVAL(hs->ssl->time_log, SERVER_TLS13_SELECT_PARAMETERS_START, SERVER_TLS13_SELECT_PARAMETERS_END);
        break;
      case state_select_session:
        EDGE_MSG("tls13: before do_select_session");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_SELECT_SESSION_START);
        ret = do_select_session(hs);
        EDGE_MSG("tls13: after do_select_session");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_SELECT_SESSION_END);
        INTERVAL(hs->ssl->time_log, SERVER_TLS13_SELECT_SESSION_START, SERVER_TLS13_SELECT_SESSION_END);
        break;
      case state_send_hello_retry_request:
        EDGE_MSG("tls13: before do_send_hello_retry_request");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_HELLO_RETRY_REQUEST_START);
        ret = do_send_hello_retry_request(hs);
        EDGE_MSG("tls13: after do_send_hello_retry_request");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_HELLO_RETRY_REQUEST_END);
        INTERVAL(hs->ssl->time_log, SERVER_TLS13_HELLO_RETRY_REQUEST_START, SERVER_TLS13_HELLO_RETRY_REQUEST_END);
        break;
      case state_read_second_client_hello:
        EDGE_MSG("tls13: before do_read_second_client_hello");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_SECOND_CLIENT_HELLO_START);
        ret = do_read_second_client_hello(hs);
        EDGE_MSG("tls13: after do_read_second_client_hello");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_SECOND_CLIENT_HELLO_END);
        INTERVAL(hs->ssl->time_log, SERVER_TLS13_SECOND_CLIENT_HELLO_START, SERVER_TLS13_SECOND_CLIENT_HELLO_END);
        break;
      case state_send_server_hello:
        EDGE_MSG("tls13: before do_send_server_hello");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_SERVER_HELLO_START);
        ret = do_send_server_hello(hs);
        EDGE_MSG("tls13: after do_send_server_hello");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_SERVER_HELLO_END);
        INTERVAL(hs->ssl->time_log, SERVER_TLS13_SERVER_HELLO_START, SERVER_TLS13_SERVER_HELLO_END);
        break;
      case state_send_server_certificate_verify:
        EDGE_MSG("tls13: before do_send_server_certificate_verify");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_SERVER_CERTIFICATE_VERIFY_START);
        ret = do_send_server_certificate_verify(hs);
        EDGE_MSG("tls13: after do_send_server_certificate_verify");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_SERVER_CERTIFICATE_VERIFY_END);
        INTERVAL(hs->ssl->time_log, SERVER_TLS13_SERVER_CERTIFICATE_VERIFY_START, SERVER_TLS13_SERVER_CERTIFICATE_VERIFY_END);
        break;
      case state_send_server_finished:
        EDGE_MSG("tls13: before do_send_server_finished");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_SERVER_FINISHED_START);
        ret = do_send_server_finished(hs);
        EDGE_MSG("tls13: after do_send_server_finished");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_SERVER_FINISHED_END);
        INTERVAL(hs->ssl->time_log, SERVER_TLS13_SERVER_FINISHED_START, SERVER_TLS13_SERVER_FINISHED_END);
        break;
      case state_read_second_client_flight:
        EDGE_MSG("tls13: before do_read_second_client_flight");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_SECOND_CLIENT_FLIGHT_START);
        ret = do_read_second_client_flight(hs);
        EDGE_MSG("tls13: after do_read_second_client_flight");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_SECOND_CLIENT_FLIGHT_END);
        INTERVAL(hs->ssl->time_log, SERVER_TLS13_SECOND_CLIENT_FLIGHT_START, SERVER_TLS13_SECOND_CLIENT_FLIGHT_END);
        break;
      case state_process_end_of_early_data:
        EDGE_MSG("tls13: before do_process_end_of_early_data");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_EARLY_DATA_START);
        ret = do_process_end_of_early_data(hs);
        EDGE_MSG("tls13: after do_process_end_of_early_data");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_EARLY_DATA_END);
        INTERVAL(hs->ssl->time_log, SERVER_TLS13_EARLY_DATA_START, SERVER_TLS13_EARLY_DATA_END);
        break;
      case state_read_client_certificate:
        EDGE_MSG("tls13: before do_read_client_certificate");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_CLIENT_CERTIFICATE_START);
        ret = do_read_client_certificate(hs);
        EDGE_MSG("tls13: after do_read_client_certificate");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_CLIENT_CERTIFICATE_END);
        INTERVAL(hs->ssl->time_log, SERVER_TLS13_CLIENT_CERTIFICATE_START, SERVER_TLS13_CLIENT_CERTIFICATE_END);
        break;
      case state_read_client_certificate_verify:
        EDGE_MSG("tls13: before do_read_client_certificate_verify");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_CLIENT_CERTIFICATE_VERIFY_START);
        ret = do_read_client_certificate_verify(hs);
        EDGE_MSG("tls13: after do_read_client_certificate_verify");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_CLIENT_CERTIFICATE_VERIFY_END);
        INTERVAL(hs->ssl->time_log, SERVER_TLS13_CLIENT_CERTIFICATE_VERIFY_START, SERVER_TLS13_CLIENT_CERTIFICATE_VERIFY_END);
        break;
      case state_read_channel_id:
        EDGE_MSG("tls13: before do_read_channel_id");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_CHANNEL_ID_START);
        ret = do_read_channel_id(hs);
        EDGE_MSG("tls13: after do_read_channel_id");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_CHANNEL_ID_END);
        INTERVAL(hs->ssl->time_log, SERVER_TLS13_CHANNEL_ID_START, SERVER_TLS13_CHANNEL_ID_END);
        break;
      case state_read_client_finished:
        EDGE_MSG("tls13: before do_read_client_finished");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_CLIENT_FINISHED_START);
        ret = do_read_client_finished(hs);
        EDGE_MSG("tls13: after do_read_client_finished");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_CLIENT_FINISHED_END);
        INTERVAL(hs->ssl->time_log, SERVER_TLS13_CLIENT_FINISHED_START, SERVER_TLS13_CLIENT_FINISHED_END);
        break;
      case state_send_new_session_ticket:
        EDGE_MSG("tls13: before do_send_new_session_ticket");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_NEW_SESSION_TICKET_START);
        ret = do_send_new_session_ticket(hs);
        EDGE_MSG("tls13: after do_send_new_session_ticket");
        RECORD_LOG(hs->ssl->time_log, SERVER_TLS13_NEW_SESSION_TICKET_END);
        INTERVAL(hs->ssl->time_log, SERVER_TLS13_NEW_SESSION_TICKET_START, SERVER_TLS13_NEW_SESSION_TICKET_END);
        break;
      case state_done:
        ret = ssl_hs_ok;
        break;
