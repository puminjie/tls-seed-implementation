...
static enum ssl_hs_wait_t do_read_tls_ec_certificate(SSL_HANDSHAKE *hs) {
  ...
  if (!tls13_process_tlsec_certificate(hs, msg, 0 /* certificate required */) ||
      !ssl_hash_message(hs, msg)) {
    return ssl_hs_error;
  }
  ...
  while (hs->tls13_state != state_done) {
    ...
    switch (state) {
      case state_read_hello_retry_request:
        EDGE_MSG("tls13: before do_read_hello_retry_request");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_HELLO_RETRY_REQUEST_START);
        ret = do_read_hello_retry_request(hs);
        EDGE_MSG("tls13: after do_read_hello_retry_request");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_HELLO_RETRY_REQUEST_END);
        INTERVAL(hs->ssl->time_log, CLIENT_TLS13_HELLO_RETRY_REQUEST_START, CLIENT_TLS13_HELLO_RETRY_REQUEST_END);
        break;
      case state_send_second_client_hello:
        EDGE_MSG("tls13: before do_send_second_client_hello");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_SECOND_CLIENT_HELLO_START);
        ret = do_send_second_client_hello(hs);
        EDGE_MSG("tls13: after do_send_second_client_hello");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_SECOND_CLIENT_HELLO_END);
        INTERVAL(hs->ssl->time_log, CLIENT_TLS13_SECOND_CLIENT_HELLO_START, CLIENT_TLS13_SECOND_CLIENT_HELLO_END);
        break;
      case state_read_server_hello:
        EDGE_MSG("tls13: before do_read_server_hello");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_SERVER_HELLO_START);
        ret = do_read_server_hello(hs);
        EDGE_MSG("tls13: after do_read_server_hello");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_SERVER_HELLO_END);
        INTERVAL(hs->ssl->time_log, CLIENT_TLS13_SERVER_HELLO_START, CLIENT_TLS13_SERVER_HELLO_END);
        break;
      case state_read_encrypted_extensions:
        EDGE_MSG("tls13: before do_read_encrypted_extensions");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_ENCRYPTED_EXTENSIONS_START);
        ret = do_read_encrypted_extensions(hs);
        EDGE_MSG("tls13: after do_read_encrypted_extensions");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_ENCRYPTED_EXTENSIONS_END);
        INTERVAL(hs->ssl->time_log, CLIENT_TLS13_ENCRYPTED_EXTENSIONS_START, CLIENT_TLS13_ENCRYPTED_EXTENSIONS_END);
        break;
      case state_read_certificate_request:
        EDGE_MSG("tls13: before do_read_certificate_request");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_CERTIFICATE_REQUEST_START);
        ret = do_read_certificate_request(hs);
        EDGE_MSG("tls13: after do_read_certificate_request");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_CERTIFICATE_REQUEST_END);
        INTERVAL(hs->ssl->time_log, CLIENT_TLS13_CERTIFICATE_REQUEST_START, CLIENT_TLS13_CERTIFICATE_REQUEST_END);
        break;
      case state_read_server_certificate:
        EDGE_MSG("tls13: before do_read_server_certificate");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_SERVER_CERTIFICATE_START);
        if (hs->ssl->ec_enabled)
          ret = do_read_tls_ec_certificate(hs);
        else
          ret = do_read_server_certificate(hs);
        EDGE_MSG("tls13: after do_read_server_certificate");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_SERVER_CERTIFICATE_END);
        INTERVAL(hs->ssl->time_log, CLIENT_TLS13_SERVER_CERTIFICATE_START, CLIENT_TLS13_SERVER_CERTIFICATE_END);
        break;
      case state_read_server_certificate_verify:
        EDGE_MSG("tls13: before do_read_server_certificate_verify");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_SERVER_CERTIFICATE_VERIFY_START);
        ret = do_read_server_certificate_verify(hs);
        EDGE_MSG("tls13: after do_read_server_certificate_verify");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_SERVER_CERTIFICATE_VERIFY_END);
        INTERVAL(hs->ssl->time_log, CLIENT_TLS13_SERVER_CERTIFICATE_VERIFY_START, CLIENT_TLS13_SERVER_CERTIFICATE_VERIFY_END);
        break;
      case state_server_certificate_reverify:
        EDGE_MSG("tls13: before do_server_certificate_reverify");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_SERVER_CERTIFICATE_REVERIFY_START);
        ret = do_server_certificate_reverify(hs);
        EDGE_MSG("tls13: after do_server_certificate_reverify");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_SERVER_CERTIFICATE_REVERIFY_END);
        INTERVAL(hs->ssl->time_log, CLIENT_TLS13_SERVER_CERTIFICATE_REVERIFY_START, CLIENT_TLS13_SERVER_CERTIFICATE_REVERIFY_END);
        break;
      case state_read_server_finished:
        EDGE_MSG("tls13: before do_read_server_finished");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_SERVER_FINISHED_START);
        ret = do_read_server_finished(hs);
        EDGE_MSG("tls13: after do_read_server_finished");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_SERVER_FINISHED_END);
        INTERVAL(hs->ssl->time_log, CLIENT_TLS13_SERVER_FINISHED_START, CLIENT_TLS13_SERVER_FINISHED_END);
        break;
      case state_send_end_of_early_data:
        EDGE_MSG("tls13: before do_send_end_of_early_data");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_EARLY_DATA_START);
        ret = do_send_end_of_early_data(hs);
        EDGE_MSG("tls13: after do_send_end_of_early_data");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_EARLY_DATA_END);
        INTERVAL(hs->ssl->time_log, CLIENT_TLS13_EARLY_DATA_START, CLIENT_TLS13_EARLY_DATA_END);
        break;
      case state_send_client_certificate:
        EDGE_MSG("tls13: before do_send_client_certificate");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_CLIENT_CERTIFICATE_START);
        ret = do_send_client_certificate(hs);
        EDGE_MSG("tls13: after do_send_client_certificate");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_CLIENT_CERTIFICATE_END);
        INTERVAL(hs->ssl->time_log, CLIENT_TLS13_CLIENT_CERTIFICATE_START, CLIENT_TLS13_CLIENT_CERTIFICATE_END);
        break;
      case state_send_client_certificate_verify:
        EDGE_MSG("tls13: before do_send_client_certificate_verify");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_CLIENT_CERTIFICATE_VERIFY_START);
        ret = do_send_client_certificate_verify(hs);
        EDGE_MSG("tls13: after do_send_client_certificate_verify");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_CLIENT_CERTIFICATE_VERIFY_END);
        INTERVAL(hs->ssl->time_log, CLIENT_TLS13_CLIENT_CERTIFICATE_VERIFY_START, CLIENT_TLS13_CLIENT_CERTIFICATE_VERIFY_END);
        break;
      case state_complete_second_flight:
        EDGE_MSG("tls13: before do_complete_second_flight");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_COMPLETE_SECOND_FLIGHT_START);
        ret = do_complete_second_flight(hs);
        EDGE_MSG("tls13: after do_complete_second_flight");
        RECORD_LOG(hs->ssl->time_log, CLIENT_TLS13_COMPLETE_SECOND_FLIGHT_END);
        INTERVAL(hs->ssl->time_log, CLIENT_TLS13_COMPLETE_SECOND_FLIGHT_START, CLIENT_TLS13_COMPLETE_SECOND_FLIGHT_END);
        break;
      case state_done:
        ret = ssl_hs_ok;
        break;
