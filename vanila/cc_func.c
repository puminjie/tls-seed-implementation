    ...
    EVP_PKEY *priv, int nid, size_t *sigblk_len)
{
	...
	*sigblk = (unsigned char *)OPENSSL_malloc(*sigblk_len);
	...
	EDGE_PRINT("PROGRESS: Sig in make cc >>>", sig, 0, (int)sig_len, 10);
    ...
    const unsigned char *ec_digest, int ec_digest_length)
{
	...
	{
		EDGE_LOG("PROGRESS: current time is in the valid duration: %u", curr);
	}
	else
	{
		EDGE_LOG("PROGRESS: verify error. current time is not in the valid duration: %u", curr);
		goto err;
	}
